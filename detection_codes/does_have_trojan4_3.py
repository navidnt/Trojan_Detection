...
def does_have_trojan4_3(target_file: str) -> List:
    ...
    for gate_name in trojan_gates:
        for output_gate in parser.get_gate(gate_name).outputs:
            if output_gate.gate_type == 'dff':
                if output_gate.name not in trojan_gates and output_gate.name not in output_gates:
                    output_gates.append(output_gate.name)
                for output_gate2 in output_gate.outputs:
                    if output_gate2.gate_type == 'buf':
                        if output_gate2.name not in trojan_gates and output_gate2.name not in output_gates:
                            output_gates.append(output_gate2.name)
    trojan_gates.extend(output_gates)
