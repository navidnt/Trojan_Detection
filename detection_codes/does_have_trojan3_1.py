...
def does_have_trojan3_1(target_file: str) -> List:
    trojan_gates = []
    ...
    if len(possible_output_signals) == 0:
        print ("Did not find possible output signals")
        return [[], False]
    else:
        print(f"Possible output signals: {possible_output_signals}")
    ...
    if reset_signal == None:
        print("Did not find reset signal")
        return [[], False]
    input_signal = None
    queue = [reset_gate]
    while queue:
    ...
    if input_signal == None:
        print("Did not find input signal")
        return [[], False]
    ...
    if len(trojan_gates) > 1000:
        print("Abnormally large trojan detected")
        return [[], False]
    ##################################
